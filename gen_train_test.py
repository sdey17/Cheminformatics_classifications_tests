'''
This script loads activity values for a protein from public databases such as BindingDB and ChEMBL.
It then combines all of those data, and removes duplicates.
Finally, it uses scaffold split to create training and test sets in a stratified manner.
'''
...
'''
1. Collect and combine the data from BindingDB and ChEMBL, remove activities with <, and include activities with > when greater than the inactive range
'''

bdb = pd.read_csv('BDB_all.csv').dropna()
chembl_EC50 = pd.read_csv('ChEMBL_EC50.csv').dropna()
chembl_IC50 = pd.read_csv('ChEMBL_IC50.csv').dropna()
chembl_Ki = pd.read_csv('ChEMBL_Ki.csv').dropna()
...
df_valid.to_csv('public_data_wActivity.csv')
...
for i in range (df_valid.shape[0]):
    if df_valid['Value'][i]<active_range:
        print(df_valid.iloc[i])
        df_valid['Activity'][i] = 1
    elif df_valid['Value'][i]>inactive_range:
        print(df_valid.iloc[i])
        df_valid['Activity'][i] = 0
...
pd.concat([train_dataset_ac_df,train_dataset_in_df]).reset_index(drop=True)[['SMILES','Activity']].to_csv('training_set_{}_{}.csv'.format(active_range, inactive_range), 
                                                                                                                         index=False)
pd.concat([test_dataset_ac_df,test_dataset_in_df]).reset_index(drop=True)[['SMILES','Activity']].to_csv('ext_test_set_{}_{}.csv'.format(active_range, inactive_range), 
                                                                                                                       index=False)